...
public class DOCReader {
	...
	public void build(ITextBuilder theBuilder) {
		...
		// TextDocumentProperties textProperties = new
		// TextDocumentProperties(this.fullpath);
		...
		for (int x = 0; x <= styleNumber; x++) {
			...
			if (desc != null) {
				...
				if (isChar) {
					// theBuilder.foundCharStyleProperties(x, basedStyleId,
					// name, cp);
				}
		...
		for (int i = 0; i <= numberParagraph - 1; i++) {
			...
			// theBuilder.foundParagraph(i, paragraphContent, pStyleUniqueID,
			// ps);
			...
			for (int iChar = 0; iChar < numTextRuns; iChar++) {
				...
				// css = new CharSequenceStyle(charStyleName,
				// uniqueCharStyleID);
